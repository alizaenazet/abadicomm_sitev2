<script lang="ts">
  ...
  const fromIndividualOrInstituionOptions = [
    "Pribadi",
    "Swasta",
    "Pemerintahan",
  ];
  let fromIndividualOrInstituion: "Pribadi" | "Swasta" | "Pemerintahan" =
    "Pribadi";

  $: isIndividual = fromIndividualOrInstituion === "Pribadi";

  const customerNeedsOptions = ["Membuat Event", "Kebutuhan Event", "Lainnya"];
  let customerNeed: "Membuat Event" | "Kebutuhan Event" | "Lainnya" =
    "Membuat Event";
  $: isHaveOtherNeed = customerNeed === "Lainnya";
  let otherNeed = "";

  $: message = (() => {
    let greeting =
      "Selamat pagi, siang, atau malam. Semoga Anda sehat dan bahagia.%0A";
    let intro = `Perkenalkan, saya ${name}`;

    if (!isIndividual) {
      intro += ` dari ${nameOfInstitution}.`;
    } else {
      intro += ".";
    }

    let purpose =
      `saya ingin ` + (isHaveOtherNeed ? otherNeed : customerNeed) + ".";
    let extraInfo = "";

    if (additionalInformation) {
      extraInfo = `%0AInformasi tambahan:%0A${additionalInformation}.`;
    }
  ...
  $: isFormFilled =
    ...
    (isIndividual || nameOfInstitution) &&
    (isHaveOtherNeed ? otherNeed : customerNeed);
  ...
  >
    ...
    >
      ...
      <form class="space-y-4">
        ...
        <div class="space-y-2">
          <label for="fromIndividualOrInstituion" class="text-sm font-medium"
            >Asal Instansi *</label
          >
          <select
            id="fromIndividualOrInstituion"
            bind:value={fromIndividualOrInstituion}
            class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {#each fromIndividualOrInstituionOptions as option}
        ...
        {#if !isIndividual}
          <div class="space-y-2">
            <label for="nama instansi" class="text-sm font-medium"
              >Nama instansi *</label
            >
            <input
              type="text"
        ...
        <div class="space-y-2">
          <label for="customerNeed" class="text-sm font-medium"
            >Keperluan *</label
          >
          <select
            id="customerNeed"
            bind:value={customerNeed}
            class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {#each customerNeedsOptions as option}
              <option value={option}>{option}</option>
            {/each}
          </select>
        </div>
        ...
        {#if isHaveOtherNeed}
          <div class="space-y-2">
            <label for="otherNeed" class="text-sm font-medium"
              >Keperluan Lainnya *</label
            >
            <input
              type="text"
              id="otherNeed"
